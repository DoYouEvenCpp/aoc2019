...
namespace {
    ...
    struct RepairRobot {
        RepairRobot(IntCodeComputer c) : pc(c)
        {
            pc.disableDebug();
        }
        ...
        void printMap() {
            ...
            for (auto& e: map) {
                const auto x_ = e.first.x + std::abs(x);
                const auto y_ = e.first.y + std::abs(y);
                data[y_][x_] = e.second;
            }
...
private:
    void makeMove(int direction, int x, int y) {
        ...
        while(!q.empty()) {
            const auto move = q.top();
            q.pop();

            auto [x_, y_] = getNewPositions(move.direction, move.x, move.y);
            const auto type = pc.run(move.direction);

            const char c = type == 0 ? '#' : type == 1 ? '.' : 'O';
            std::cout << '(' << move.x << ", " << move.y << ") -> (" << y_ << ", " << x_ << "): " << c << '\n';
            if (type == 0) {
                addPoint(x_, y_, c);
                continue;
            }
            ...
            const auto oppositeDirection = getOpositeDirection(move.direction);
        ...
        while (!moves.empty()) {
            if (pc.run(moves.top()) != 1) std::cout << "ZONK\n";
            moves.pop();
        }
    }
