...
std::tuple<int64_t, int64_t> runTests(int64_t param) {
    return run(param);
}

std::tuple<int64_t, int64_t> run(int64_t param) {
    bool shouldBrake = false;
    std::tuple<int64_t, int64_t> answer;
    while(true) {
        ...
        switch(opCode) {
            ...
            case OpCodes::OUTPUT: {
                ...
                // shouldBrake = true;
                std::cout << val << ' ';
                if ((counter++ % 2) == 0) shouldBrake = true;
                ip += 2;
                break;
            }
