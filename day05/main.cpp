...
const auto runProgram = [](int param) {
    ...
    for (auto i = 0u; i < input.size();) {
        ...
        if (opCode == OpCodes::ADD) {
            ...
            index += 4;
            const auto res = code.param_modes[0] == 1 ? input[firstOperandPos]: firstOperandPos
            + code.param_modes[1] == 1? input[secondOperandPos] : secondOperandPos;
            const auto pos = code.param_modes[2] == 1 ? input[resultPos] : resultPos;
            input[pos] = res;
            std::cout << "ADD: input[" << pos << "] = " << res << '\n';
        }
            ...
            * code.param_modes[1] == 1? input[secondOperandPos] : secondOperandPos;
            ...
            const auto pos = code.param_modes[2] == 1 ? input[resultPos] : resultPos;
            input[pos] = res;
            std::cout << "MULTIPLY: input[" << pos << "] = " << res << '\n';
        }
        else if (opCode == OpCodes::STORE) {
            const auto firstOperandPos = input[index + 1];
            const auto pos = code.param_modes[0] == 1 ? input[firstOperandPos] : firstOperandPos;
            input[pos] = param;
            index += 2;
