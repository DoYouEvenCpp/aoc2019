...
const auto runProgram = [](int firstParam, int secondParam, std::vector<int>& input) {
    ...
    for (auto i = 0u; i < input.size();) {
        ...
        if (opCode == OpCodes::ADD) {
            ...
            input[pos] = !firstParamUsed ? firstParam : secondParam;
            if (!firstParamUsed) firstParamUsed = true;
            index += 2;
...
struct Amp {
    ...
    int process(int inputParam) {
        return runProgram(phase, inputParam, mInput);
    }
...
int main()
{
    std::vector<int> thrusters = { 0,1,2,3,4 };
    auto sum = 0;
    ...
    do {
        ...
        const auto temp_sum = amps[4].process(amps[3].process(amps[2].process(amps[1].process(amps[0].process(0)))));
        if (temp_sum > sum) {
            sum = temp_sum;
            answer = thrusters;
        }
