...
int main()
{
    ...
    std::map<int, std::list<Packet>> router;
    ...
    while(shallRun) {
        for (auto i = 0; i < 50; ++i) {
            ...
            auto [val, lastOpCode, address] = network[i].run(param);
            if (address == 255){
                shallRun = router[255].size() > 0 && router[255].begin()->y > 0;
            }
            //find first packet, which isn't fully fetched
        }
    }
