	...
	for (auto& e: secondWire) {
		...
		for (auto& other: first) {
			if (doIntersect(s._start, s._end, other._start, other._end)) {
				...
				auto diff = 0u;
				if (e.direction == Mov::D || e.direction == Mov::U) diff = std::abs(s._end.y - point.y);
				else if (e.direction == Mov::L || e.direction == Mov::R) diff = std::abs(s._end.x - point.x);
				std::cout << "Second(" << point.x << ", " << point.y << "): "
	...
	for (auto& e : firstWire) {
		...
		for (auto& other : second) {
			if (doIntersect(s._start, s._end, other._start, other._end)) {
				...
				std::cout << "First(" << point.x << ", " << point.y << "): "
					<< calculateDistance(0, 0, point.x, point.y)
					<< " - " << firstSum - diff << '\n';
			}
		}
